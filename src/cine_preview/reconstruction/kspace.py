...
def sort_kspace(scan: BrukerScan) -> npt.NDArray[Any]:
    ...
    # FlowEncLoop is present on phase-contrast / TPM acquisitions (4-step or
    # 9-step Hadamard encoding) and absent on CINE — default to 1 for CINE.
    flow_enc_dir = int(np.asarray(method.get("FlowEncLoop", 1)).ravel()[0])
...
def _place_cs_lines(
    ...
    CSPhaseEncList holds 1-indexed, fractional phase-encode positions: values
    are integer multiples of ``8 / PVM_Matrix(1)`` spanning roughly ``[-4, 4]``
    (so the count of distinct positions equals ``PVM_Matrix(1)``). The
    1-indexed integer k-space column is ``round((list + 4) * PVM_Matrix(1) / 8)``;
    subtract 1 for 0-indexing. Verified on AGORA scans: CINE matrix 128
    → multiplier 16 (matches the legacy hardcoded value); TPM matrix 96
    → multiplier 12.
    ...
    matrix_x = int(np.asarray(method["PVM_Matrix"]).ravel()[0])
    multiplier = matrix_x / 8.0
    cs_indices = np.rint((cs_raw.astype(float) + 4.0) * multiplier).astype(int) - 1
