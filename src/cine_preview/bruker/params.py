...
def read_param_string(path: Path, field: str) -> str | None:
    """Read a single string-valued JCAMP field without parsing the whole file.

    Detection only needs ``ACQ_scan_name``; running the full
    :func:`parse_bruker_params` (numpy arrays, struct arrays, RLE expansion for
    *every* parameter) just to pull one field is wasted work on every scan.
    This streams the file, stops at the first matching ``##$<field>=`` line,
    gathers its continuation lines and returns the first ``<…>`` token with the
    surrounding angle brackets stripped.

    Args:
        path:  Path to a JCAMP-DX file (e.g. ``acqp``).
        field: Parameter name without the ``##$`` prefix or ``=``.

    Returns:
        The field's string value, or ``None`` if the field is absent or holds
        no ``<…>`` token. Raises ``OSError`` if the file cannot be opened
        (callers distinguish "missing" from "unparsable").
    """
    prefix = f"##${field}="
    with open(path, "r", encoding="latin-1") as fh:
        ...
        for line in lines:
            ...
            # String arrays declare a size first (e.g. "( 64 )") with the
            # <…> value on following continuation lines; collect until closed.
            while ">" not in buffer:
                nxt = next(lines, None)
